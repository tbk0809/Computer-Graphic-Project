...
void myKeyboardFunc(unsigned char key, int x, int y) {
    ...
    // We removed caDefendingAnim and imDefendingAnim from here so they don't lock the game between turns!
    bool isAnimating = myvirtualworld.imAttacking || myvirtualworld.imAttackingMissiles ||
                       myvirtualworld.caAttackingHammer || myvirtualworld.caAttackingSuper;

    // Check if the game is over
    bool isGameOver = (myvirtualworld.imHP <= 0 || myvirtualworld.caHP <= 0);

    // ==========================================
    // 1. UNIVERSAL CONTROLS (Always active)
    // ==========================================
    switch (key) {
    ...
    // ==========================================
    // 2. TURN-BASED COMBAT CONTROLS
    // ==========================================
    // Only allow moves/attacks if the game is running AND no animation is playing
    if (!isAnimating && !isGameOver) {

        // --- PLAYER 1: IRON MAN'S TURN ---
        if (myvirtualworld.currentTurn == 1) {
            switch(key) {
                ...
                case ' ': // Basic Attack
                    ...
                    cout << "\n[Iron Man] Casts Unibeam Blast!" << endl;
                    break;
                case 'q': case 'Q': // Missile Attack
                    myvirtualworld.imAttackingMissiles = true;
                    myvirtualworld.imAttackTimer = 240;
                    cout << "\n[Iron Man] Deploys Micro-Missiles!" << endl;
                    break;
                case 'e': case 'E': // Defend Action
                    myvirtualworld.imIsDefending = true;
                    myvirtualworld.imDefendingAnim = true;
                    myvirtualworld.imAttackTimer = 60; // Quick 60-frame setup animation
                    cout << "\n[Iron Man] Deploys Energy Forcefield!" << endl;
                    break;

                case 'h': case 'H': xinc=setting.posInc; break; // 'h' just moves world during Iron Man's turn
            }
        }
        // --- PLAYER 2: CAPTAIN AMERICA'S TURN ---
        else if (myvirtualworld.currentTurn == 2) {
            switch(key) {
                ...
                case 13: // Enter - Defend
                    myvirtualworld.caIsDefending = true;
                    myvirtualworld.caDefendingAnim = true;
                    myvirtualworld.caAttackTimer = 60; // Quick setup animation
                    cout << "\n[Captain America] Raises Shield to Defend!" << endl;
                    break;
                case 'u': case 'U': // Ultimate
                    myvirtualworld.caAttackingSuper = true;
                    myvirtualworld.caAttackTimer = 540;
                    cout << "\n[Captain America] Casts Thunder Hammer Super!" << endl;
                    break;
                case 'h': case 'H': // Hammer
    ...
    // Explicit check to catch lowercase 'h'
    if ((key == 'h' || key == 'H') && !myvirtualworld.caDefendingAnim && !myvirtualworld.caAttackingHammer && !myvirtualworld.caAttackingSuper && myvirtualworld.caHP > 0) {
        myvirtualworld.caAttackingHammer = true;
        myvirtualworld.caAttackTimer = 240;
        cout << "\n[Captain America] Casts Hammer Strike!" << endl;
    }
...
void myWelcome() {
    ...
    cout<<"|                  Q for Micro-Missiles                         |\n";
    cout<<"|   Captain Am. -> I/J/K/L to move, ENTER for Shield Attack     |\n";
    cout<<"|                  H to throw Hammer, U for Hammer Super        |\n";
    cout<<"|                                                               |\n";
    cout<<"| World:                                                        |\n";
